...
const ToasterLoading = ({ onLoadingComplete, duration = 5000 }) => {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setProgress((prev) => {
        ...
        if (Math.floor(newProgress) % 20 === 0 && Math.floor(prev) % 20 !== 0) {
          playTickSound();
        }
        ...
        // Toast popping animation at 100%
        if (newProgress >= 100 && !isPopping) {
          setIsPopping(true);
          playDingSound();
  ...
  }, [duration, isPopping, showSteam, onLoadingComplete, toastControls, toasterControls]);
          ...
          >
            ...
            >
              ...
              >
                ...
                {/* Butter on top (when popped) */}
                {isPopping && (
                  <motion.div
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                    transition={{ delay: 0.2 }}
                    className="absolute -top-2 left-1/2 transform -translate-x-1/2 w-12 h-3 bg-yellow-300 rounded-full shadow-lg"
                  />
                )}
              </motion.div>
            </motion.div>
